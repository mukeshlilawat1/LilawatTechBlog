...
public class EmailServiceImpl implements EmailService {
    ...
    public void sendOtpEmail(String email, String otp) {
        try {
            ...
            helper.setText(buildEmailTemplate(otp), true); // ✅ FIX
    ...
    public void sendPasswordResetEmail(String email, String otp) {
        try {
            ...
            MimeMessageHelper helper = new MimeMessageHelper(message, true);
            helper.setTo(email);
            helper.setSubject("Password Reset - LilawatTechBlog");
            helper.setText(buildPasswordResetTemplate(otp), true);
            mailSender.send(message);
        } catch (Exception e) {
            throw new RuntimeException("Failed to send email: " + e.getMessage());
        }
    ...
    private String buildPasswordResetTemplate(String otp) {
        return """
                <div style="font-family: Arial, sans-serif; max-width: 500px; margin: 0 auto;">
                    <h2 style="color: #333;">Password Reset Request</h2>
                    <p>Your OTP for password reset is:</p>
                    <div style="background: #f4f4f4; padding: 20px; text-align: center; border-radius: 8px;">
                        <h1 style="color: #dc3545; letter-spacing: 8px;">%s</h1>
                    </div>
                    <p style="color: #666;">This OTP will expire in <strong>5 minutes</strong>.</p>
                    <p style="color: #999; font-size: 12px;">If you didn't request this, please ignore this email.</p>
                </div>
